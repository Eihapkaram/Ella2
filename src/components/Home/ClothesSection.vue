<template>
  ...
  <div>
    <v-slide-group>
      <v-slide-group-item v-for="pro in this.catigoryProducts4" :key="pro.id">
      </v-slide-group-item>
    </v-slide-group>
  </div>

  <div style="height: 25rem; margin-top: 50px; position: relative">
    <header id="head">
      <h3 class="ml-5 text-h5">dresses<img style="width: 30px" src="" /></h3>
      <v-spacer></v-spacer>
      <div
        class="progress"
        role="progressbar"
    ...
    >
      <swiper-slide
        id="swip"
        v-for="pro in this.catigoryProducts4"
        :key="pro.id"
      >
        <v-card id="card">
          <v-row>
...
export default {
  ...
  async mounted() {
    await this.getCatigoryProduct4("womens-dresses");
  ...
  methods: {
    ...
    ...mapActions(mystore, ["getCatigoryProduct4"]),
    async lovefun(data) {
  ...
  computed: {
    ...mapState(mystore, ["catigoryProducts4"]),

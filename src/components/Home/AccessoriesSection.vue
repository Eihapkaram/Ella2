<template>
  ...
  <div>
    <v-slide-group>
      <v-slide-group-item v-for="pro in this.catigoryProducts2" :key="pro.id">
      </v-slide-group-item>
    </v-slide-group>
  </div>

  <div style="height: 38re; margin-top: 50px; position: relative">
    <header id="head">
      <h3 class="ml-5 text-h5" style="cursor: pointer">
        Accessories<img style="width: 30px" src="" />
      </h3>
      <v-spacer></v-spacer>
      <div
        class="progress"
    ...
    >
      <swiper-slide
        id="swip"
        v-for="pro in this.catigoryProducts2"
        :key="pro.id"
      >
        <v-card id="card">
          <v-row>
    ...
    <div class="d-flex" style="position: relative" width="80%" id="photocon">
      <img id="im" src="@/assets/images/band-right-cover.webp" />
      <v-spacer></v-spacer>
      <img id="im" src="@/assets/images/band-left-cover.webp" />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  async mounted() {
    await this.getCatigoryProduct2("mobile-accessories");
  ...
  methods: {
    ...
    ...mapActions(mystore, ["getCatigoryProduct2"]),
    async lovefun(data) {
  ...
  computed: {
    ...mapState(mystore, ["catigoryProducts2"]),
